...
impl CliCommand for DevCommand {
    async fn handle(&self, ctx: &CliContext) -> anyhow::Result<CommandOutput> {
        ...
        let artifact_path = match build_component(&self.project_dir, ctx, &config).await {
            ...
            Err(e) => {
                // TODO(GFI): Support continuing, npm start works like that.
        ...
        // TODO(GFI): Only spawn the server if the component exports wasi:http
        ...
        let mut watcher = notify::recommended_watcher(move |res: _| match res {
            Ok(event) => {
                ...
                {
                    // TODO(GFI): ensure we ignore the artifact dirs
        ...
        loop {
            ...
            select! {
                ...
                _ = reload_rx.recv() => {
                    ...
                    // TODO(GFI): Skip wit fetch if no .wit change
                    // TODO(GFI): Typescript: Skip install if no package.json change
