...
impl crate::plugin::bindings::wasmcloud::wash::types::Host for SharedCtx {}
/// The Context resource is a passthrough to the same map we use for runtime configuration
impl crate::plugin::bindings::wasmcloud::wash::types::HostContext for SharedCtx {
    async fn get(&mut self, ctx: Resource<Context>, key: String) -> Option<String> {
        let context = match self.active_ctx.table.get(&ctx) {
            Ok(context) => context,
            Err(e) => {
...
impl crate::plugin::bindings::wasmcloud::wash::types::HostProjectConfig for SharedCtx {
    async fn version(&mut self, ctx: Resource<ProjectConfig>) -> String {
        let c = self.active_ctx.table.get(&ctx).unwrap();
        c.version.clone()
    }
...
impl crate::plugin::bindings::wasmcloud::wash::types::HostRunner for SharedCtx {
    ...
    async fn plugin_config(
        ...
        let Some(plugin) = self
            .active_ctx
            .get_plugin::<PluginManager>(PLUGIN_MANAGER_ID)
        else {
        ...
        let runtime_config = {
            ...
            match all_config.get(&self.active_ctx.id) {
                Some(config) => config.clone(),
                None => {
                    tracing::warn!(component_id = %self.active_ctx.id, "no plugin config found, returning default");
    ...
    async fn host_exec(
        ...
        let ctx_data = self.active_ctx.table.get(&ctx).map_err(|e| e.to_string())?;
...
impl crate::plugin::bindings::wasmcloud::wash::types::HostPluginConfig for SharedCtx {
    async fn get(&mut self, ctx: Resource<PluginConfig>, key: String) -> Option<String> {
        let plugin_config = match self.active_ctx.table.get(&ctx) {
    ...
    async fn set(
        ...
        let plugin_config = match self.active_ctx.table.get_mut(&ctx) {
            Ok(plugin_config) => plugin_config,
            Err(e) => {
                tracing::error!(error = %e, "failed to get plugin config resource");
                return None;
