...
use wash_runtime::{
    ...
    plugin::{
        wasi_config::WasiConfig, wasi_grpc_client::GrpcClient, wasi_http::HttpServer,
...
async fn test_grpc_client_basic() -> Result<()> {
    ...
    // Start HTTP server for the component
    let http_port = find_available_port().await?;
    let http_addr: SocketAddr = format!("127.0.0.1:{http_port}").parse().unwrap();

    // Create engine and host with both HTTP server and gRPC client plugins
    let engine = Engine::builder().build()?;
    let grpc_client = GrpcClient::new();

    println!("✓ Created gRPC client plugin");
    println!("  - Protocol: HTTP/2 only (h2c for cleartext)");
    println!("  - Auto-adds Content-Type: application/grpc header");
    println!("  - Connection pooling: Enabled\n");

    let host = HostBuilder::new()
        .with_engine(engine.clone())
        .with_plugin(Arc::new(HttpServer::new(http_addr)))?
        .with_plugin(Arc::new(grpc_client))?
