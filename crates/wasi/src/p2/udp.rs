...
impl LoopbackIncomingDatagramStream {
    pub fn recv(
        &mut self,
        datagrams: &mut Vec<crate::p2::bindings::sockets::udp::IncomingDatagram>,
        max_results: usize,
    ) -> Result<(), crate::sockets::util::ErrorCode> {
        let Ok(mut rx) = self.rx.try_lock() else {
            return Err(crate::sockets::util::ErrorCode::Unknown);
        };
        ...
        while datagrams.len() < max_results {
            ...
            match self.remote_address {
                ...
                _ => datagrams.push(crate::p2::bindings::sockets::udp::IncomingDatagram {
                    data: dgram.data,
