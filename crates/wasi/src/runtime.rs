...
#[derive(Debug)]
pub struct AbortOnDropJoinHandle<T>(Option<tokio::task::JoinHandle<T>>);

impl<T> AbortOnDropJoinHandle<T> {
    /// Extract the inner JoinHandle without aborting
    pub fn into_inner(mut self) -> tokio::task::JoinHandle<T> {
        self.0.take().expect("JoinHandle already taken")
    }
    ...
    pub async fn cancel(mut self) -> Option<T> {
        let mut handle = self.0.take().expect("JoinHandle already taken");
        handle.abort();
...
impl<T> Future for AbortOnDropJoinHandle<T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let handle = self.0.as_mut().expect("JoinHandle already taken");
